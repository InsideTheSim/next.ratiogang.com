...
function createPriceFeed() {
    ...
    if (
        userCurrency &&
        siteOptions.getAvailableCurrencies().includes(userConfig.currency.id.toUpperCase())
    ) {
        ...
        coinbaseWSS.onopen = () => {
            coinbaseWSS.send(JSON.stringify({
                type: 'subscribe',
                product_ids: tickers,
        ...
        // once we have enough data to calculate the ratio,
        // we can close the ticker feed and only use the batch feed
        ratio.$on('current', () => {
            if (!bootstrapped) {
                coinbaseWSS.send(JSON.stringify({
                    type: 'unsubscribe',
